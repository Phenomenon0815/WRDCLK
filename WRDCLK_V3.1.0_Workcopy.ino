...
//
// - Bei Wechsel der Wörter aus und einfaden
//
// TODO: 
...
void fade_out() {
  ...
  leds[7,110].fadeToBlackBy(brightness_value);
}

void fade_in() {
  //leds[i].fadeLightBy( 64 );  
  leds[7,110].fadeLightBy(brightness_value);
}

void TageszeitAuswerten() {
  if ((hour() >= 7)&&(hour() <= 18)) {    // Zwischen 07:00 und 18:59
    brightness_value_max = 145;
    brightness_value = 145;
    if ((hour() == 7 && minute() == 0)) { // Licht an
      LichtAn();
    }
  }
  ...
  if (hour() == 23) {                     // Zwischen 23:00 und 23:59
    brightness_value_max = 40;
    brightness_value = 40;
    if (minute() == 0) {                  // Licht aus
      LichtAus();
    }
  }
