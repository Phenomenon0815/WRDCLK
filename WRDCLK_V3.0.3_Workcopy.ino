...
// - Ab 23 Uhr Beleuchtung auf aus, ab 7 Uhr an
...
void LichtAn() {
  if (toggle == false) {
  // for (brightness_value_temp = 0; brightness_value_temp < brightness_value_max; brightness_value_temp ++) {
      LEDS.setBrightness(brightness_value_max);
      FastLED.show();
      DEBUG_PRINTLN("Funktion: Licht anschalten...");
  }
  else if (toggle == true) {
    LEDS.setBrightness(brightness_value_max);
    FastLED.show();
    DEBUG_PRINTLN("Funktion: Licht bleibt an...");
  }
}
...
void LichtAus() {
  if (toggle == true) {
  // for (brightness_value_temp = brightness_value_max; brightness_value_temp > 0; brightness_value_temp --) {
      LEDS.setBrightness(brightness_value_low);
      FastLED.show();
      DEBUG_PRINTLN("Funktion: Licht ausschalten...");
  }
  else if (toggle == false) {  
     LEDS.setBrightness(brightness_value_low);
     FastLED.show();
     DEBUG_PRINTLN("Funktion: Licht bleibt aus...");
  }
}
